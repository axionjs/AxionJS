...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-[50vh] md:min-h-[40vh] flex items-center justify-center overflow-hidden">
      <Container className="relative z-10">
        ...
        >
          ...
          <motion.p
            variants={itemVariants}
            className="text-lg md:text-xl text-muted-foreground mb-12 max-w-3xl mx-auto leading-7 [&:not(:first-child)]:mt-6"
          >
          ...
          <motion.div
            variants={itemVariants}
            className="hidden md:flex flex-col md:flex-row items-center justify-center space-y-4 md:space-y-0 md:space-x-12 text-sm text-muted-foreground"
          >
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(6)].map((_, i) => (
            <motion.div
              key={i}
              className="absolute w-20 h-20 border border-border/20 rounded-lg"
              animate={{
                y: [0, -20, 0],
                rotate: [0, 180, 360],
                opacity: [0.3, 0.6, 0.3],
