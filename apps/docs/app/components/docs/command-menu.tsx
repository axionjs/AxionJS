...
import { type Color, type ColorPalette } from "@/lib/color";
import { source } from "@/lib/source";
import { cn } from "@/lib/utils";
import { useConfig } from "@/hooks/use-config";
import { useIsMac } from "@/hooks/use-is-mac";
import { useMutationObserver } from "@/hooks/use-mutation-observer";
import { copyToClipboardWithMeta } from "@/app/components/docs/copy-button";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="secondary"
          className={cn(
            "bg-surface text-surface-foreground/60 dark:bg-card relative h-8 w-full justify-start pl-2.5 font-normal shadow-sm sm:pr-12 md:w-40 lg:w-56 xl:w-64"
          )}
      ...
      >
        ...
        <Command className="**:data-[slot=command-input-wrapper]:bg-input/50 **:data-[slot=command-input-wrapper]:border-input rounded-none bg-transparent **:data-[slot=command-input]:!h-9 **:data-[slot=command-input]:py-0 **:data-[slot=command-input-wrapper]:mb-0 **:data-[slot=command-input-wrapper]:!h-9 **:data-[slot=command-input-wrapper]:rounded-md **:data-[slot=command-input-wrapper]:border">
          ...
          <CommandList className="no-scrollbar min-h-80 scroll-pt-2 scroll-pb-1.5">
            ...
            {colors?.map((colorPalette, paletteIndex) => (
              <CommandGroup
                key={`${colorPalette.name}-${paletteIndex}`}
                heading={
