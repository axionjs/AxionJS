...
type TextEffectProps = {
  ...
  className?: string
  preset?: "fade-in" | "fade-in-up" | "fade-in-down" | "fade-in-blur" | "typewriter"
  delay?: number
  duration?: number
...
export function TextEffect({
  ...
  useEffect(() => {
    if (per === "character") {
      ...
      tempDiv.style.fontSize = window.getComputedStyle(containerRef.current).fontSize
      tempDiv.style.fontFamily = window.getComputedStyle(containerRef.current).fontFamily
      tempDiv.style.fontWeight = window.getComputedStyle(containerRef.current).fontWeight
      tempDiv.style.letterSpacing = window.getComputedStyle(containerRef.current).letterSpacing
      tempDiv.style.lineHeight = window.getComputedStyle(containerRef.current).lineHeight
  ...
  return (
    <Component ref={containerRef} className={className} {...props}>
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="inline">
        {segments.length > 0 ? (
