...
export function InfiniteSlider({
  ...
  }, [contentWidth, speed, speedOnHover, direction, hovering, pauseOnHover, gap, controls])
  ...
  return (
    ...
    >
      <motion.div className="flex" style={{ x, gap: `${gap}px` }} animate={controls}>
        ...
        {Array.from({ length: numDuplicates }).map((_, i) => (
          <div key={`duplicate-${i}`} className="flex shrink-0" style={{ gap: `${gap}px` }}>
            {children}
          </div>
        ))}
      </motion.div>
