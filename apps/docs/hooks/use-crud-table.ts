...
import { mockProductService } from "@/lib/crud-table-mock-data";
import type { Product } from "@/lib/crud-table-mock-data";
...
export function useCrudTable(initialPageSize = 5) {
  ...
  const handleUpdate = async (
    ...
    try {
      await mockProductService.updateProduct(productData);
      await fetchData();
      return { success: true };
    } catch (err) {
      console.error(err);
