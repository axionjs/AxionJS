...
export async function transformTailwindConfig(input, tailwindConfig, config) {
...
export async function _createSourceFile(input, config) {
  const dir = await fs.mkdtemp(path.join(tmpdir(), "axionjs-"));
  const resolvedPath =
    config?.resolvedPaths?.tailwindConfig || "tailwind.config.ts";
  const tempFile = path.join(dir, `axionjs-${path.basename(resolvedPath)}`);
...
export function _getQuoteChar(configObject) {
  return configObject
...
export function nestSpreadProperties(obj) {
...
export function nestSpreadElements(arr) {
...
export function unnestSpreadProperties(obj) {
  const properties = obj.getProperties();
...
export function unnsetSpreadElements(arr) {
  const elements = arr.getElements();
  for (let j = 0; j < elements.length; j++) {
    const element = elements[j];
    if (element.isKind(SyntaxKind.ObjectLiteralExpression)) {
  ...
  for (const property of node.getProperties()) {
    if (property.isKind(SyntaxKind.PropertyAssignment)) {
      ...
      if (
        ...
        result[name] = parseObjectLiteralExpression(property.getInitializer());
      } else if (
        property.getInitializer()?.isKind(SyntaxKind.ArrayLiteralExpression)
      ) {
        result[name] = parseArrayLiteralExpression(property.getInitializer());
      } else {
...
export function buildTailwindThemeColorsFromCssVars(cssVars) {
  const result = {};
